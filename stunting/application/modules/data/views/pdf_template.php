...
<head>
    <meta charset="UTF-8">
    <title>Laporan Data Pengukuran</title>
    <style>
        body {
...
<body>
    <h1>Laporan Data Pengukuran Balita</h1>

    <div class="container">
        <span>Nama Petugas:</span> <?= $user['name']; ?> <br>
        <span>Tempat Pengukuran:</span> <?= $user['desa']; ?> <?= $user['kecamatan']; ?> <?= $user['kabupaten']; ?> <?= $user['provinsi']; ?>

        <div class="data-section">
            <h2>Identitas Balita</h2><br>
            <span><b>Nama:</b></span> <?= $pengukuran->nama_balita; ?><br>
            <span><b>Jenis Kelamin:</b></span> <?= $pengukuran->gender2; ?><br>
            <span><b>Tanggal Lahir:</b></span> <?= date('d-m-Y', strtotime($pengukuran->lahir)); ?><br>
            <span><b>Orang Tua:</b></span> <?= $pengukuran->nama_ortu; ?><br>
            <span><b>Alamat:</b></span> <?= $pengukuran->alamat2; ?><br>
        </div>

        <div class="data-section">
            <h2>Pengukuran</h2><br>
            <span><b>Tanggal Pengukuran:</b></span> <?= date('d-m-Y', strtotime($pengukuran->date_create)); ?><br>
            <span><b>Umur:</b></span>
            <?php
            $birth_date = new DateTime($pengukuran->lahir);
            $measurement_date = new DateTime($pengukuran->date_create);
            $age_interval = $measurement_date->diff($birth_date);
            echo $age_interval->y . ' tahun, ' . $age_interval->m . ' bulan';
            ?>
            <br>
            <span><b>Faktor Aktivitas:</b></span> <?= $pengukuran->aktivitas; ?><br>
            <span><b>Faktor Stress:</b></span> <?= $pengukuran->stress; ?><br>
            <span><b>Berat Badan:</b></span> <?= $pengukuran->mass; ?> Kg<br>
            <span><b>Tinggi Badan:</b></span> <?= $pengukuran->height; ?> cm<br>
            <span><b>Lingkar Kepala:</b></span> <?= $pengukuran->head; ?> cm<br>
        </div>
        ...
        <div class="data-section">
            ...
            // Pastikan untuk mengambil elemen array yang benar sesuai posisi ID di URI
            $id_url = isset($uri_segments[4]) ? $uri_segments[4] : null;
            foreach ($data as $u) {
                if ($u->id == $id_url) {
        ...
        <div class="data-section">
            ...
            echo "5. Total Kebutuhan Kalori = <b> $tdee kkal/hari</b>";
            ?>
        </div>

        <div class="footer">
            © SHIELD Dibuat Oleh Sistem Pendeteksi Dini Stunting <?= date('d-m-Y'); ?>
        </div>
    </div>
</body>
