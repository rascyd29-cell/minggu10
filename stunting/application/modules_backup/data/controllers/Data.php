...
class Data extends MX_Controller
{
    ...
    {
        // Melakukan join antara data_pengukuran dan tb_balita untuk mendapatkan nama balita
        ...
        $datas['data2'] = $this->db->select('*')->from('historysesnor_topi')->limit(1)->order_by('id', 'DESC')->get()->result();
        $datas['data2_'] = $this->db->select('*')->from('historysesnor_timbangan')->limit(1)->order_by('id', 'DESC')->get()->result();
        $datas['title'] = 'Pengukuran';
    ...
    function add()
    {
        ...
        $isi = array(
            'name' => $this->input->post('id_balita'),
            'aktivitas' => $this->input->post('aktivitas'),
            'stress' => $this->input->post('stress'),
            'gender' => $this->input->post('id_balita'),
            'parents' => $this->input->post('id_balita'),
            'address' => $this->input->post('id_balita'),
        ...
        $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Pengukuran Berhasil ditambahkan!</div>');
        redirect('data');
    }
    ...
    public function hapus()
    {
        ...
        $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Data Pengukuran Berhasil Dihapus!</div>');
        redirect('data');
    }
    ...
    public function rekomendasi($id)
    {
        ...
        $this->template->load('template1', 'data/spk', $data);
    }
    ...
    public function export_pdf($id)
    {
        ...
        $html = $this->load->view('data/pdf_template', $data, true);
    ...
    function riwayat()
    {
        $datas['title'] = 'Riwayat Sensor';
        $datas['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
        $datas['data'] = $this->db->select('*')->from('historysesnor_timbangan')->order_by('id', 'DESC')->get()->result(); //Untuk mengambil data dari database webinar
        $datas['data2'] = $this->db->select('*')->from('historysesnor_timbangan')->limit(1)->order_by('id', 'DESC')->get()->result(); //Untuk mengambil data dari database webinar
        $datas['data_'] = $this->db->select('*')->from('historysesnor_topi')->order_by('id', 'DESC')->get()->result(); //Untuk mengambil data dari database webinar
        $datas['data2_'] = $this->db->select('*')->from('historysesnor_topi')->limit(1)->order_by('id', 'DESC')->get()->result(); //Untuk mengambil data dari database webinar
    ...
    public function reset_data()
    {
        ...
        $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">History Sensor berhasil dihapus!</div>');
    ...
    function orangtua()
    {
        $data['data'] = $this->db->get_where('user', ['role_id' => 2])->result(); // Ambil hanya akun orang tua
        ...
        $this->template->load('template1', 'data/list', $data);
    ...
    function edit2()
    {
        if (isset($_POST['submit'])) {
            ...
            $datas['title'] = 'Orang Tua';
            $datas['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();

            $this->load->view('templates/header', $datas);
            $this->load->view('templates/sidebar', $datas);
            $this->load->view('templates/topbar', $datas);
            $this->template->load('template1', 'data/edit2', $datas);
        }
    }
    ...
    {
        // Ambil hanya akun orang tua
        $data['data'] = $this->db->get_where('user', ['role_id' => 2])->result();

        // Melakukan join antara tb_balita dan user untuk mendapatkan nama orang tua
        $this->db->select('tb_balita.*, user.name as nama_ortu');
        $this->db->from('tb_balita');
        $this->db->join('user', 'user.id = tb_balita.id_ortu', 'left');
        $data['balita'] = $this->db->get()->result();
        ...
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->template->load('template1', 'data/balita', $data);
    }
    ...
    function add_balita()
    {
        ...
        $this->db->insert('tb_balita', $isi);
        $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Balita Berhasil ditambahkan!</div>');
        redirect('data/balita');
    }

    public function search_orang_tua()
    {
        $query = $this->input->post('query');
        $this->db->like('name', $query); // Mencari di kolom 'name'
        $result = $this->db->get('user')->result();

        $output = '';
        foreach ($result as $user) {
            $output .= '<div class="suggestion-item" data-id="' . htmlspecialchars($user->id) . '" style="padding: 10px; cursor: pointer;">' . htmlspecialchars($user->name) . '</div>';
        }
    ...
    {
        // Ambil data balita berdasarkan ID dengan join ke tabel user untuk mendapatkan nama orang tua
        $this->db->select('tb_balita.*, user.name as nama_ortu');
        $this->db->from('tb_balita');
        $this->db->join('user', 'user.id = tb_balita.id_ortu', 'left');
        $this->db->where('tb_balita.id', $id);  // Filter berdasarkan ID balita
        $data['balita'] = $this->db->get()->row_array();  // Ambil satu baris data

        // Ambil data orang tua untuk saran
        $this->db->select('id, name');
        $data['ortu'] = $this->db->get_where('user', ['role_id' => 2])->result_array();
        ...
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->template->load('template1', 'data/edit_balita', $data);
    }

    public function update_balita($id)
    {
        // Validasi dan ambil data dari input
        $data_update = [
            'nama' => $this->input->post('nama'),
            'jenis_kelamin' => $this->input->post('jenis_kelamin'),
            'tanggal_lahir' => $this->input->post('tanggal_lahir'),
        ...
        $this->db->where('id', $id);
        $this->db->update('tb_balita', $data_update);

        // Set flashdata untuk notifikasi
        $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Data balita berhasil diupdate!</div>');

        // Redirect ke halaman data balita
        redirect('data/balita');
    }


    function hapus_balita()
    {
        $id = $this->uri->segment(3);
        if (!empty($id)) {
            // proses delete data
        ...
        $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Nama Balita Dihapus!</div>');
        redirect('data/balita');
    }
}
