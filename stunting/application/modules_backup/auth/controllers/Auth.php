...
class Auth extends CI_Controller
{
    ...
    public function rest()
    {
        ...
        $cek = $this->db->from('user')->where('HWID', $HWID)->where('password', $password)->get()->num_rows();
        if ($cek != null) {
            echo 'Sukses';
        } else {
            echo 'Gagal';
    ...
    public function registration()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Selamat! Akun Anda telah dibuat. Harap aktifkan akun Anda!</div>');
    ...
    private function _sendEmail($token, $type)
    {
        $config = [
            ...
            'smtp_user' => 'portablemyfarm@gmail.com',
            'smtp_pass' => 'sznc rqbg ootn izsp',
            'smtp_port' => 465,
            'mailtype'  => 'html',
            'charset'   => 'utf-8',
            'newline'   => "\r\n"
        ...
        if ($type == 'verify') {
            $this->email->subject('Formulir Verifikasi Akun Deteksi Dini Stunting');
            $this->email->message('Klik tautan ini untuk memverifikasi akun Anda : <a href="' . base_url() . 'auth/verify?email=' . $this->input->post('email') . '&token=' . urlencode($token) . '">Aktifkan</a>');
        } else if ($type == 'forgot') {
            $this->email->subject('Formulir Reset Password Deteksi Dini Stunting');
            $this->email->message('Klik tautan ini untuk mengatur ulang kata sandi Anda: <a href="' . base_url() . 'auth/resetpassword?email=' . $this->input->post('email') . '&token=' . urlencode($token) . '">Reset Password</a>');
        }
