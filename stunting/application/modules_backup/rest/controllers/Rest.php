...
class Rest extends MX_Controller
{
    ...
    function jason_login_orangtua($ortu_id = null)
    {
        // Melakukan join antara tb_balita, user, dan data_pengukuran untuk mendapatkan informasi yang diperlukan
        $this->db->select('tb_balita.*, user.name as nama_ortu, user.email as email_ortu, data_pengukuran.*');
        $this->db->from('tb_balita');
        $this->db->join('user', 'user.id = tb_balita.id_ortu', 'left'); // Join untuk mendapatkan nama dan email orang tua
        $this->db->join('data_pengukuran', 'data_pengukuran.name = tb_balita.id', 'left'); // Join dengan data_pengukuran

        // Tambahkan kondisi untuk hanya memilih user dengan role_id = 2
        $this->db->where('user.role_id', 2);

        // Jika $ortu_id diberikan, tambahkan kondisi where
        if ($ortu_id !== null) {
            $this->db->where('tb_balita.id_ortu', $ortu_id); // Filter by ortu_id
        }

        // Mengambil data dan urutkan berdasarkan id secara descending
        $datas = $this->db->order_by('tb_balita.id', 'DESC')->get()->result();


        $response = array("Data" => array());

        if ($ortu_id !== null) {
            // Hitung jumlah data_pengukuran yang name-nya sesuai dengan id balita yang di-filter
            $this->db->where('data_pengukuran.name IN (SELECT id FROM tb_balita WHERE id_ortu = ' . $ortu_id . ')');
            $jumlah_data_pengukuran = $this->db->count_all_results('data_pengukuran');
        } else {
            $jumlah_data_pengukuran = 0;
        }
        ...
        foreach ($datas as $r) {
            $temp = array(
                ...
                "nama_balita" => $r->nama,
                "id_balita" => $r->id,
                "jumlah_balita" => count($datas),
                "jumlah_pengukuran" => $jumlah_data_pengukuran
